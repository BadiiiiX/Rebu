...
public class RideInDatabaseDAO extends AbstractDAO implements RideDAO {
    ...
    public List<RideInterface> findAll() {
        ...
        try (Statement stmt = connection.createStatement();
            ...
            while (rs.next()) {
                ...
                ride.getPassengers().addAll(findByPassengerId(rideId));
    ...
    @Override
    public void save(RideInterface ride) {
        String sql = """
    ...
    public List<RideInterface> findByPassengerId(int passengerId) {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                int rideId = rs.getInt("ride_id");
                CarInterface vehicle = findVehicleById(rs.getInt("vehicle_id"));
                UserInterface driver = findUserById(rs.getInt("driver_id"));
                String origin = rs.getString("origin");
                String destination = rs.getString("destination");
                Date startDate = new Date(rs.getTimestamp("start_date").getTime());
    ...
    @Override
    public void removePassengerFromRide(int rideId, long passengerId) {
